...
export default function StayEditor({snapshot,initialId,onClose,onSave}:Props) {
  ...
  const cities = [...new Set([...base.places.map(p=>p.city),...ordered.map(s=>s.city)])].filter(Boolean).sort();
  ...
  return <dialog ref={dialog} className="stay-editor" aria-labelledby="stay-editor-heading" onCancel={onClose}>
    ...
    {review ? <div className="stay-review">
      ...
      {!!review.errors.length && <section className="stay-error" aria-label="Problems to fix"><h3>Adjust these dates</h3><ul>{review.errors.map((text,i)=><li key={i}>{text}</li>)}</ul></section>}
      ...
      {editing ? <form ref={form} onSubmit={e=>{e.preventDefault();preview();}} className="stay-fields">
        ...
        <label>City<input required list="stay-city-options" value={editing.city} onChange={e=>setEditing({...editing,city:e.target.value})}/></label><datalist id="stay-city-options">{cities.map(city=><option key={city} value={city}/>)}</datalist>
